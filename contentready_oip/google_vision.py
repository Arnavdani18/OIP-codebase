...
def is_file_explicit(file_url):
    try:
        ...
        with open(file_path, 'rb') as f:
            content = f.read()
            return is_content_explicit(content)
    except Exception as e:
        print("is_file_explicit:\n\n",str(e))
        raise

@frappe.whitelist(allow_guest=False)
def is_base64_explicit(b64str):
    import base64
    content = base64.decodebytes(b64str.encode('utf-8'))
    return is_content_explicit(content)

def is_content_explicit(content):
