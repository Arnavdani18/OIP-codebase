...
frappe.ready(async () => {
  ...
  attachFeaturedBtn = (file) => {
    ...
    let found = frappe.web_form.doc.media.find((m) =>
      m.attachment.endsWith(file.name)
    );
    ...
    <div class="d-flex justify-content-center" data-feature="${file.name}">
      <button class="close" title="featured photo">
        <i class="fa ${
          found && found['is_featured'] === 1 ? 'fa-bookmark' : 'fa-bookmark-o'
        }" aria-hidden="true"></i>
      </button>
    </div>`);
    ...
    $(`div[data-feature="${file.name}"]`).on('click', function (e) {
      ...
      let currentMediaIndex = frappe.web_form.doc.media.findIndex((m) =>
        m.attachment.endsWith(currentMediaName)
      );
      ...
      for (const section of $('div[data-feature]')) {
        ...
        if (mediaName === currentMediaName) {
          ...
          let index = frappe.web_form.doc.media.findIndex((m) =>
            m.attachment.endsWith(mediaName)
          );
