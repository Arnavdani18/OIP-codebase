...
def get_context(context):
    ...
    names = [d['name'] for d in matched[context.start:context.end]]
    filtered = frappe.get_list('Organisation', filters={'name': ['in', names], 'type': 'Service Provider'})
    context.organisations = [frappe.get_doc('Organisation', d['name']) for d in filtered]
    return context